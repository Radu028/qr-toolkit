def detectPositioning(img, height, width):
    eyes = [[0,0],[0,0]]
...
def getModule(img, height, width):
    eyes = detectPositioning(img, height, width)
...
def findCoordonates(img, height, width):
    module_size=getModule(img, height, width)
...
def positionedQR(qr, img, height, width): #Rotates the QR if needed
